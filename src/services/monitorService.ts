...
import { Event, Monitor, MonitorResponse, type Report } from 'src/models'
import { MonitorProducer, Network } from 'src/data'
import { type MonitorDetail, ReportService } from 'src/services'
import { getResponseTime } from 'src/utils'
import { validateMonitor } from 'src/validators'

// export class MonitorService {

export const notifyCreateMonitor = async (monitor: Monitor): Promise<void> => {
...
export const notifyDeleteMonitor = async (monitor: Monitor): Promise<void> => {
  if (NODE_ENV === 'development') return
 ...
 */
export const createMonitor = async (data: any): Promise<Monitor> => {
